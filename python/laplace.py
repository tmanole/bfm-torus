...
for i in range(n1):
    for j in range(n2):
        ...
        if v < 0.25:
            phi[i,j] = np.exp(-1 / (1- 4*v   )) -1 #np.cos(x) + np.sin(y)
...
#phi = np.cos(2*np.pi*x) + np.cos(2*np.pi*y)

Lx = 1 
dx = Lx/(N-1)

fphi = np.fft.fft2(phi,axes=[0,1])

omega_x = np.fft.fftfreq(fphi.shape[0], d=dx)
omega_y = np.fft.fftfreq(fphi.shape[1], d=dx)
print(omega_x)
print(omega_y)
lapl = np.empty((n1,n2))
...
#            print(omega_x[i]**2+omega_y[j]**2)
            lapl[i,j] = fphi[i,j] / (omega_x[i]**2 + omega_y[j]**2)

iphi = np.fft.ifft2(lapl,axes=[0,1])
